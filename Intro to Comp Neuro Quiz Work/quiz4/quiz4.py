"""
Q7 – Plotting tuning curves

Here we load a pickle file with firing rate data from 4 neurons across
different stimulus directions. Each neuron's firing rates are trial-by-trial,
so we average across trials to get the mean response per stimulus. Finally,
we plot tuning curves (mean firing rate vs. stimulus direction) for each
neuron to visualize their preferred orientations.
"""


"""
Q8 – Poisson check for spike count variability

This script checks whether the neurons' firing rates follow Poisson-like
statistics. For a Poisson process, the variance of spike counts should
roughly equal the mean (Var/Mean ≈ 1/T for rates). We load the same tuning
data, compute Var/Mean across stimuli for each neuron, and then compare
to the theoretical expectation (≈0.1 here, since T=10s). Finally, we report
which neuron deviates the most from Poisson statistics.
"""


"""
Q9 – Population vector decoding

Here I decode a stimulus direction from the responses of 4 neurons.
Steps:
1. Load tuning curves and a separate file with "mystery stimulus" responses.
2. For each neuron, compute r_max (max mean firing rate across stimuli).
3. Compute each neuron's mean response (r̄) to the mystery stimulus.
4. Each neuron has a preferred direction vector c_i (basis).
5. Build the population vector: sum_i (r̄_i / r_max_i) * c_i.
6. Decode the angle from that vector (convert to quiz’s coordinate system).

The result is the predicted stimulus direction, both exact and rounded
